...
func parseWorkspaceListArgs(args []string) (workspaceListOptions, error) {
	fs := flag.NewFlagSet("workspace list", flag.ContinueOnError)
	fs.SetOutput(io.Discard)
	jsonOutput := fs.Bool("json", false, "emit structured JSON")
	if err := fs.Parse(args); err != nil {
		return workspaceListOptions{}, err
	}
	if fs.NArg() != 0 {
		return workspaceListOptions{}, fmt.Errorf("workspace list does not accept positional arguments")
	}
	return workspaceListOptions{JSON: *jsonOutput}, nil
}
...
func runWorkspaceList(args []string) int {
	options, err := parseWorkspaceListArgs(args)
	if err != nil {
	...
	if options.JSON {
		payload := map[string]any{"workspaces": workspaceListJSON(items)}
		if err := json.NewEncoder(os.Stdout).Encode(payload); err != nil {
			fmt.Fprintf(os.Stderr, "workspace list: encode JSON: %v\n", err)
			return 1
		}
		return 0
	}
...
func workspaceListJSON(items []workspace.Workspace) []map[string]any {
