...
func TestWorkspaceListDoesNotRequireRemoteSession(t *testing.T) {
	...
	first := items[0]
	second := items[1]
	firstID, _ := first["workspace_id"].(string)
	secondID, _ := second["workspace_id"].(string)
	if firstID == "" || secondID == "" || firstID == secondID {
		t.Fatalf("workspace ids must be stable and distinct: first=%+v second=%+v", first, second)
	}
...
func TestWorkspaceUnregisterInvalidatesExistingSessionWorkspaceBinding(t *testing.T) {
	runtime := newWorkspaceRuntime(t, "alpha")
	...
	if statusOK(resumed) || envelopeErrorCode(resumed) != "WORKSPACE_UNREGISTERED" {
		t.Fatalf("existing Session must fail closed after unregister: %+v", resumed)
	}
}
