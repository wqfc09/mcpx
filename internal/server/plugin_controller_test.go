...
func TestControllerBindHolderRejectsDetachedLease(t *testing.T) {
	manager := newControllerRuntimeManager(nil)
	lease := &controllerRuntimeLease{Key: "workspace:w1:Coordinator", holders: map[string]bool{}}
	manager.leases[lease.Key] = lease
	delete(manager.leases, lease.Key) // model natural exit/controllerExited before Acquire binds
	if manager.bindHolder(lease, "session:s1") {
		t.Fatal("detached Controller lease accepted a lifecycle holder")
	}
	lease.stateMu.Lock()
	defer lease.stateMu.Unlock()
	if len(lease.holders) != 0 {
		t.Fatalf("detached lease gained orphan holders: %#v", lease.holders)
	}
}

func TestControllerStartLockSerializesSameLeaseKey(t *testing.T) {
	manager := newControllerRuntimeManager(nil)
	firstEntered := make(chan struct{})
	releaseFirst := make(chan struct{})
	firstDone := make(chan struct{})
	go func() {
		unlock := manager.acquireStartLock("workspace:w1:Coordinator")
		close(firstEntered)
		<-releaseFirst
		unlock()
		close(firstDone)
	...
	select {
	case <-secondEntered:
		t.Fatal("same Controller lease key entered creation concurrently")
	...
	case <-time.After(time.Second):
		t.Fatal("second Controller candidate did not enter after first released start lock")
	}
	<-firstDone
	<-secondDone
}

func TestControllerStateCleanupDoesNotRemoveSuccessorLease(t *testing.T) {
	runtimeDir := t.TempDir()
	manager := newControllerRuntimeManager(nil)
	key := "workspace:w1:Coordinator"
	oldLease := &controllerRuntimeLease{Key: key, Plugin: "Coordinator", RuntimeDir: runtimeDir, Generation: "old"}
	newLease := &controllerRuntimeLease{Key: key, Plugin: "Coordinator", RuntimeDir: runtimeDir, Generation: "new"}
	manager.leases[key] = newLease
	if err := pluginstate.Write(runtimeDir, pluginstate.Status{
		Plugin: "Coordinator", Runtime: config.PluginRuntimeNative, Scope: config.PluginScopeWorkspace,
		PID: 4321, Executable: "/bin/controller", Generation: newLease.Generation,
	}); err != nil {
		t.Fatal(err)
	}

	manager.removeControllerStateIfCurrent(oldLease)
	status, err := pluginstate.Read(runtimeDir)
	if err != nil {
		t.Fatalf("old Controller cleanup removed successor state: %v", err)
	}
	if status.Generation != newLease.Generation {
		t.Fatalf("successor generation=%q want=%q", status.Generation, newLease.Generation)
	}

	manager.removeControllerStateIfCurrent(newLease)
	if _, err := pluginstate.Read(runtimeDir); !os.IsNotExist(err) {
		t.Fatalf("current Controller cleanup did not remove owned state: %v", err)
	}
}
...
func TestControllerPluginCoordinatesMountedMCPAndPinsContribution(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if controllerLease != nil {
			...
			if kinds["mount_result"] && kinds["session_mount_result"] && controllerHasSessionDependencyEvent(controllerEvents, "JEA", remoteID) {
				break
			}
		}
	...
	for _, item := range items {
		if item["plugin"] == "Coordinator" && item["kind"] == "controller_started" && item["delivery"] == "deferred" {
			foundNative = true
		}
	}
	if !foundNative {
		t.Fatalf("aggregate Inbox did not include Native Plugin event: %+v", items)
	}
	if inboxData["schema_version"] != float64(3) || inboxData["timed_out"] != false {
		t.Fatalf("aggregate Inbox V3 metadata=%+v", inboxData)
	}
	...
	if !statusOK(closed) {
		t.Fatalf("close Coordinator Session=%+v", closed)
	}
	ws := rt.workspaceRuntime("demo", workspacePath)
	if state := rt.controllerLeases.State("Coordinator", ws); state["state"] != "configured" {
		t.Fatalf("Controller remained after last Session holder released: %+v", state)
	}
	for _, dependency := range []string{"Comet", "JEA"} {
		state := rt.pluginLeases.State(dependency, config.PluginScopeWorkspace, ws)
		if state["state"] != "configured" {
			t.Fatalf("Controller dependency %s remained after all holders released: %+v", dependency, state)
		}
	}
}
